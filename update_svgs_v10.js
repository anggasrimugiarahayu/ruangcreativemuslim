...
function replaceSvg(idx, newSvg) {
  ...
  // Find the nth '<g transform="translate(821 600)">'
  while (count < idx) {
  ...
  // Find the closing </svg> that follows it
  let endIdx = page.indexOf('</svg>', startIdx);
  
  // The prefix is everything before the <g>
  const prefix = page.substring(0, startIdx);
  // The suffix is everything from </svg> onwards
